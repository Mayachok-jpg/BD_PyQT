...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.clear_history = QAction(QIcon('cat-poo-icon.png'), 'Очистить историю', self)
        # exitAction = QAction(QIcon('cat-poo-icon.png'), 'Poo', self)
        # self.clear_history.setShortcut('Ctrl+Q')
        # self.clear_history.triggered.connect(qApp.quit)
...
class ClearHistoryWindow(QDialog):
    def __init__(self):
        super().__init__()
        self.initUI()
    ...
    def initUI(self):
        # Настройки окна
        self.setFixedSize(365, 100)
        self.setWindowTitle('Подтверждение очистки истории')

        # Кнопка подтверждения
        self.ok_btn = QPushButton('Очистить', self)
        self.ok_btn.move(90, 40)

        # Кнопка закрытия окна
        self.close_button = QPushButton('Отмена', self)
        self.close_button.move(175, 40)
        self.close_button.clicked.connect(self.close)
